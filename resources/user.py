...
class UserRegister(Resource):
    ...
    def post(self):
        ...
        uuid = uuid1()
        newUser = UserModel(str(uuid), **data)
        if newUser.find_by_user_email(data['user_email']):
...
class User(Resource):
    ...
    def get(cls, user_email):
        ...
        return userModel.json(user.user_email, user.first_name, user.last_name)
...
class UserLogin(Resource):
    ...
    def post(cls):
        ...
        user = userModel.find_by_user_email(request_data['user_email'])
        if user and safe_str_cmp(user.user_password, request_data['user_password']):
            access_token = create_access_token(
                identity=user.user_uuid, fresh=True)
